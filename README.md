...
**L'email non e scritta nell'HTML.** In `index.html` il `<body>` porta
`data-email-user` e `data-email-domain`, e `script.js` ricompone l'indirizzo a
runtime riempiendo gli elementi marcati `data-email-link` (il `href` mailto) e
`data-email-text` (il testo visibile). Serve a ridurre la raccolta automatica
di indirizzi. Per cambiarla basta modificare i due attributi sul `<body>`.

Di conseguenza l'email non compare nemmeno nei dati strutturati JSON-LD: li
c'e solo il telefono.
